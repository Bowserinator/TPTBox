...
void HUD::drawText(const char *text, int x, const int y, const Color color, const bool ralign) const {
    ...
    DrawRectangle(x - PAD_X, y - PAD_Y, tsize.x + PAD_X * 2, tsize.y + 2 * PAD_Y, Fade(BLACK, 0.5f));
...
void HUD::displayTooltip(const char *text) {
    ...
    if (strlen(text) > MAX_TOOLTIP_LENGTH)
        throw std::invalid_argument(
            TextFormat("Tooltip '%s' exceeds max length of %i characters", text, MAX_TOOLTIP_LENGTH));
#endif
...
void HUD::updateControls(const BrushRenderer &brush_renderer) {
    ...
    // Display modes
    constexpr int NUM_KEYS[] = {KEY_ONE, KEY_TWO,   KEY_THREE, KEY_FOUR, KEY_FIVE,
    ...
    if (consumeKey) EventConsumer::ref()->consumeKeyboard();
}
...
void HUD::draw(const HUDData &data) {
    ...
    if (rx >= 0 && ry >= 0 && rz >= 0) {
        ...
        if (ID(sim->photons[rz][ry][rx])) idx = ID(sim->photons[rz][ry][rx]);
    }
    ...
    if (showHUD && debug && data.brush_renderer->brush_in_sim()) {
        ...
        drawText(TextFormat("%d, %d, %d / d%d / s%d, %d, %d", brush_pos.x, brush_pos.y, brush_pos.z,
                            data.brush_renderer->get_offset(), bsize.x, bsize.y, bsize.z),
                 GetMouseX() + 20.0f, GetMouseY() + 20.0f, WHITE);
    }

    // Bottom left: camera coords + movement mode
    if (showHUD) {
        drawText(TextFormat("Cam: %d, %d, %d %s", (int)std::round(cam->camera.position.x),
                            (int)std::round(cam->camera.position.y), (int)std::round(cam->camera.position.z),
        ...
        for (int line = 0; line < 2; line++) {
            ...
            const auto tsize =
                text_format::measure_text_ex(FontCache::ref()->main_font, text.c_str(), FONT_SIZE, FONT_SPACING);
            const float x = 20.0f;
            const float y = GetScreenHeight() - 120.0f - tsize.y - (tsize.y + 2 * PAD_Y) * (line + 1);
            DrawRectangle(x - PAD_X, y - PAD_Y, tsize.x + PAD_X * 2, tsize.y + 2 * PAD_Y, Fade(BLACK, 0.5f));
            text_format::draw_text_colored_ex(FontCache::ref()->main_font, text, Vector2{x, y}, FONT_SIZE, FONT_SPACING,
                                              WHITE);
        }
    }
    ...
    if (showHUD) {
        ...
        const char *text =
            !debug ? TextFormat("FPS: %.3f", avgFPS())
        ...
        if (state == HUDState::DEBUG_MODE)
            drawText(TextFormat("Sim: %.3f  Thrd: %d", avgSimFPS(), sim->actual_thread_count), 20, 20 + OFFSET,
                     BLUE_TEXT);
        ...
        const char *line11 = idx ? TextFormat("%s%s", name.c_str(), ctype_data.c_str()) : "Empty";
        ...
        for (std::size_t i = 0; i < profiler::profilers.size(); i++) {
            drawTextRAlign(profiler::summarize(i).c_str(), GetScreenWidth() - RHUD_X_OFFSET, 60 + (3 + i) * OFFSET,
                           YELLOW);
        }
#endif
        ...
        if (idx) {
            const char *dcolor =
                sim->parts[idx].dcolor.as_RGBA() ? TextFormat("#%08X", sim->parts[idx].dcolor.as_RGBA()) : "0";
            ...
            drawTextRAlign(TextFormat("Temp: %.2f %c  Life: %d, tmp1: %d, tmp2: %d, dcolor: %s", formattedTemp,
                                      tempUnit, sim->parts[idx].life, sim->parts[idx].tmp1, sim->parts[idx].tmp2,
                                      dcolor),
                           GetScreenWidth() - RHUD_X_OFFSET, 20 + OFFSET, WHITE);

            if (debug) {
                // Top right corner
                drawTextRAlign(TextFormat("VEL: %.2f, %.2f, %.2f, flag: %s", sim->parts[idx].vx, sim->parts[idx].vy,
                                          sim->parts[idx].vz, sim->parts[idx].flag.to_string().c_str()),
        ...
        // Air HUD
        {
            const int x = util::clamp(rx, SIM_PADDING, XRES - SIM_PADDING);
            const int y = util::clamp(ry, SIM_PADDING, YRES - SIM_PADDING);
            const int z = util::clamp(rz, SIM_PADDING, ZRES - SIM_PADDING);
            int air_rows_down = 1;
            if (idx)
    ...
    if (tooltip_opacity) {
        ...
        DrawTextEx(FontCache::ref()->main_font, tooltip,
                   Vector2{(float)(GetScreenWidth() / 2 - tsize.x / 2), (float)(GetScreenHeight() / 2)}, FONT_SIZE,
                   FONT_SPACING, Color{255, 255, 255, (uint8_t)(tooltip_opacity * 255)});

        tooltip_opacity -= FrameTime::ref()->getDelta() / TOOLTIP_TIME_SECONDS;
        if (tooltip_opacity < 0.01) tooltip_opacity = 0.0;
    }
}
