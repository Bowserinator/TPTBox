...
#include "../resources/shaders/generated/brush_clip.vs.h"
        ...
        util::set_shader_value(brush_model_shader, brush_model_shader_locs.get("simRes"),
...
#pragma omp parallel for
    ...
    BrushFaceModels out{.models = {}, .size = size};
            ...
            if (i == 0) {
                for (int y = 0; y < size.y; y++) { // Front / back
                    for (int x = 0; x < size.x; x++) {
                        ...
                        // Back faces
                        z = 0;
                        while (!is_filled(x, y, z) && z <= size.z) {
                            z++;
                        }
                        if (z <= size.z) {
                            add_vertex(BrushFace::Z, x, y, z);
                ...
                for (int z = 0; z < size.z; z++) {
                    for (int y = 0; y < size.y; y++) { // Left / right
                        ...
                        // Right faces
                        x = 0;
                        while (!is_filled(x, y, z) && x <= size.x) {
                            x++;
                        }
                        if (x <= size.x) {
                            add_vertex(BrushFace::X, x, y, z);
                ...
                for (int z = 0; z < size.z; z++) { // Top / bottom
                    for (int x = 0; x < size.x; x++) {
                        ...
                        // Bottom faces
                        y = 0;
                        while (!is_filled(x, y, z) && y <= size.y) {
                            y++;
                        }
                        if (y <= size.y) {
                            add_vertex(BrushFace::Y, x, y, z);
        ...
        for (int z = 0; z <= size.z; z++) {
            for (int y = 0; y <= size.y; y++) {
                for (int x = 0; x <= size.x; x++) {
                    // Top faces
                    int x2 = x;
                    if (!is_filled(x2, y, z) && is_filled(x2, y - 1, z)) x2++;
                    if (x2 > x) {
                        add_vertex(BrushFace::Y, x, y, z);
                        add_vertex(BrushFace::Y, x, y, z + 1);
                    ...
                    // Left faces
                    int y2 = y;
                    if (!is_filled(x, y2, z) && is_filled(x - 1, y2, z)) y2++;
                    if (y2 > y) {
                        add_vertex(BrushFace::X, x, y, z);
                    ...
                    // Front faces
                    x2 = x;
                    if (!is_filled(x2, y, z) && is_filled(x2, y, z - 1)) x2++;
                    if (x2 > x) {
                        add_vertex(BrushFace::Z, x, y, z);
...
void BrushFaceModels::draw(Vector3T<int> center, Renderer *renderer, bool deleteMode) {
    const Vector3 centerOfModel =
        Vector3{static_cast<float>(center.x - (int)size.x / 2), static_cast<float>(center.y - (int)size.y / 2),
                static_cast<float>(center.z - (int)size.z / 2)};

    SetShaderValueMatrix(brush_model_shader, brush_model_shader_locs.get("mv"),
                         MatrixTranslate(centerOfModel.x, centerOfModel.y, centerOfModel.z));
    util::set_shader_value(brush_model_shader, brush_model_shader_locs.get("resolution"),
                           Vector2{(float)GetScreenWidth(), (float)GetScreenHeight()});
    rlEnableShader(brush_model_shader.id);
    rlSetUniformSampler(brush_model_shader_locs.get("depth"), renderer->get_base_tex().depthTexture);
    rlDisableShader();
    ...
    Vector3 boundingBoxOffset =
        Vector3{size.x % 2 == 1 ? 0.5f : 0.0f, size.y % 2 == 1 ? 0.5f : 0.0f, size.z % 2 == 1 ? 0.5f : 0.0f};
