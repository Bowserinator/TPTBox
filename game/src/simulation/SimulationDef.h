...
constexpr unsigned int XRES   = 176;
constexpr unsigned int YRES   = 176;
constexpr unsigned int ZRES   = 176;
...
constexpr int SIM_PADDING = 4; // Before: 1, need to add padding because air cell boundaries do not update

constexpr bool BOUNDS_CHECK(int x, int y, int z) {
    return (x >= SIM_PADDING) & (x < (int)(XRES - SIM_PADDING)) & (y >= SIM_PADDING) & (y < (int)(YRES - SIM_PADDING)) &
           (z >= SIM_PADDING) & (z < (int)(ZRES - SIM_PADDING));
}
constexpr bool REVERSE_BOUNDS_CHECK(int x, int y, int z) {
    return (x < SIM_PADDING) | (x >= (int)(XRES - SIM_PADDING)) | (y < SIM_PADDING) | (y >= (int)(YRES - SIM_PADDING)) |
           (z < SIM_PADDING) | (z >= (int)(ZRES - SIM_PADDING));
}
constexpr uint32_t FLAT_IDX(coord_t x, coord_t y, coord_t z) { return x + y * XRES + z * (XRES * YRES); }
