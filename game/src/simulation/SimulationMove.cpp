...
void Simulation::move_behavior(const part_id idx) {
    ...
    if (el.Gravity) {
        switch (gravity_mode) {
        case GravityMode::ZERO_G: break;
        case GravityMode::VERTICAL:
            if (eval_move(idx, x, y - 1, z) != PartSwapBehavior::NOOP) part.vy -= el.Gravity;
            break;
        case GravityMode::RADIAL:
            gravity_radial_neighbors_occupied =
                TYP(pmap[z - 1][y][x]) == part.type && TYP(pmap[z + 1][y][x]) == part.type &&
                TYP(pmap[z][y][x - 1]) == part.type && TYP(pmap[z][y][x + 1]) == part.type &&
                TYP(pmap[z][y + 1][x]) == part.type && TYP(pmap[z][y - 1][x]) == part.type;
    ...
    if (el.State == ElementState::TYPE_LIQUID || el.State == ElementState::TYPE_POWDER) {
        ...
        switch (gravity_mode) {
        case GravityMode::ZERO_G: return;
        case GravityMode::VERTICAL:
            {
                if (!el.Diffusion) return;
                if (y > 1 && eval_move(idx, x, y - 1, z) != PartSwapBehavior::NOOP) // Particle can move down
                    return;

                if ( // No neighboring spots anyways, terminate
                    TYP(pmap[z - 1][y][x]) == part.type && TYP(pmap[z - 1][y][x - 1]) == part.type &&
                    TYP(pmap[z - 1][y][x + 1]) == part.type && TYP(pmap[z][y][x - 1]) == part.type &&
                    TYP(pmap[z][y][x + 1]) == part.type && TYP(pmap[z + 1][y][x]) == part.type &&
                    TYP(pmap[z + 1][y][x + 1]) == part.type && TYP(pmap[z + 1][y][x - 1]) == part.type)
                    return;
                ...
                for (int i = 0; i < el.Diffusion; i++) {
                    ...
                    if (REVERSE_BOUNDS_CHECK(x + dx, newy, z + dz)) return;

                    const float newyf     = is_liquid ? part.y : part.y - 1.0f;
                    bool can_move_y_check = is_liquid || (eval_move(idx, x + dx, y, z + dz) != PartSwapBehavior::NOOP);
                    ...
                    if (can_move_y_check) {
                        ...
                        if (std::abs(dx) > 1.0f || std::abs(dz) > 1.0f) {
                            auto pmapOccupied = [idx, this](const Vector3T<signed_coord_t> &loc) -> PartSwapBehavior {
                                if (REVERSE_BOUNDS_CHECK(loc.x, loc.y, loc.z)) return PartSwapBehavior::NOOP;
                                if (TYP(pmap[loc.z][loc.y][loc.x]) == parts[idx].type) return PartSwapBehavior::SWAP;
                                return eval_move(idx, loc.x, loc.y, loc.z);
                            };

                            RaycastOutput out;
                            hit = raycast(
                                RaycastInput{.x = x, .y = y, .z = z, .vx = (float)dx, .vy = 0.0f, .vz = (float)dz}, out,
                                pmapOccupied);

                            if (hit) try_move(idx, out.x, newyf, out.z, out.move);
                        }
        ...
        case GravityMode::RADIAL:
            {
                ...
                if (is_liquid || eval_move(idx, x + util::sign(gravity_force.x), y + util::sign(gravity_force.y),
                                           z + util::sign(gravity_force.z)) != PartSwapBehavior::NOOP) {
 ...
 * @param z Target z
 */
void Simulation::try_move(const part_id idx, const float tx, const float ty, const float tz,
                          PartSwapBehavior behavior) {
    const coord_t x = util::roundf(tx);
    const coord_t y = util::roundf(ty);
    const coord_t z = util::roundf(tz);

    // TODO: consider edge mode
    if (REVERSE_BOUNDS_CHECK(x, y, z)) return;
    ...
    if (behavior == PartSwapBehavior::NOT_EVALED_YET) behavior = eval_move(idx, x, y, z);

    switch (behavior) {
    case PartSwapBehavior::NOOP: return;
    case PartSwapBehavior::SWAP:
        {
            const auto destId = ID(part_map[z][y][x]);
            if (destId)
                swap_part(x, y, z, oldx, oldy, oldz, destId, idx);
            else
                move_part(x, y, z, idx);
    ...
    case PartSwapBehavior::OCCUPY_SAME:
        ...
        heat.update_temperature(x, y, z, heat.heat_map[oldz][oldy][oldx], GetElements()[parts[idx].type].HeatConduct);
 ...
 */
void Simulation::swap_part(const coord_t x1, const coord_t y1, const coord_t z1, const coord_t x2, const coord_t y2,
    ...
    if (part1_is_e || !photons[z1][y1][x1]) _set_color_data_at(x1, y1, z1, &parts[id2]);
    if (part2_is_e || !photons[z2][y2][x2]) _set_color_data_at(x2, y2, z2, &parts[id1]);
...
void Simulation::_raycast_movement(const part_id idx, const coord_t x, const coord_t y, const coord_t z) {
    ...
    if (no_move || !hit) {
        ...
        simd_util::clamp3f_ip_full(new_pos.x, new_pos.y, new_pos.z, SIM_PADDING, SIM_PADDING, SIM_PADDING,
                                   XRES - 1.0f - SIM_PADDING, YRES - 1.0f - SIM_PADDING, ZRES - 1.0f - SIM_PADDING);
...
PartSwapBehavior Simulation::eval_move(const part_id idx, const coord_t nx, const coord_t ny, const coord_t nz) const {
    ...
    if (can_move[this_type][other_type] != PartSwapBehavior::SPECIAL) return can_move[this_type][other_type];
